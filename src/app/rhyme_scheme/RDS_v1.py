...
class Rhyme_detection:
    ...
    def preprocess(self):
        ...
        m=0
        self.finalwords = [[0 for k in range(n)] for j in range(len(lastwords))]
    ...
    def identical_rhymes(self):
        for i in range(len(self.finalwords)):
            for j in range(i+1,len(self.finalwords)):
                if(self.finalwords[i][1]==self.finalwords[j][1]):
                    self.finalwords[j][3]=alphabet[self.count]
                    self.finalwords[i][3]=alphabet[self.count]
                    self.count+=1
                ...
                for j in range(len(self.finalwords[x][1][0])-1,0,-1):
                    ...
                    if(counter>2):
                        if(self.finalwords[x][3]==0 and self.finalwords[i][3]==0):
                            self.finalwords[x][3]=alphabet[self.count]
                            self.finalwords[i][3]=alphabet[self.count]
                            self.count+=1
                            print("Eye rhymes are:", self.finalwords[x][1], self.finalwords[i][1])
                        counter=0
    def display(self):
        a=0
