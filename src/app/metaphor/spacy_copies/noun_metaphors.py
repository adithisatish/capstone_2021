...
class NounMetaphor(MetaphorUtil):
    ...
    def __init__(self, text = "", sp_w = 0.465 ,wp_w = 0.023, threshold = 0.1456265):#0.977, 
        self.text = text
    ...
    def train(self,data, true_values):
        ...
        self.spacy_weight = optimal_weight

    def test(self, data, true_values):
        scores = []
        predictions = []
        vals = list(true_values)
        ...
        for i, text in enumerate(data):
                ...
                self.dependencies = {}
                pred,score = self.noun_metaphor_util(doc, code=1)
                print("Score: {0} \t Predicted: {1} \t Actual: {2}".format(score, pred, vals[i]))

                predictions.append(pred)
                scores.append(score)

        print("\n\nWeights - Spacy: {0}".format(self.spacy_weight))
